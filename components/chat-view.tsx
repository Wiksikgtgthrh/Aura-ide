            ...
            {error && (
              <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive" role="alert">
                {(() => {
                  ...
                  // Перегруз интерфейса при стриме (исторический баг до
                  // включения throttle в useChat). Отдельная ветка — заодно
                  // маркер сборки: старая версия показывала английский хвост.
                  if (/maximum update depth/i.test(msg)) {
                    return (
                      <span>
                        Интерфейс перегрузился при обработке стрима — известная проблема,
                        исправленная включением батчинга обновлений. Обновите страницу
                        (Ctrl+Shift+R) и повторите. Если сообщение появляется снова на свежей
                        версии — пришлите стек из консоли браузера (строка «[aura] chat stream error»).
                      </span>
                    )
                  }
