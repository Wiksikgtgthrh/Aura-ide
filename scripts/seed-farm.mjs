...
const PLUGIN = {
  ...
  manifest: JSON.stringify({
    ...
    whereItAppears:
      'Кнопка «V0 Farm» в сайдбаре (админам и пользователям с выданными ключами) открывает диалог генерации. Управление ключами, группами, моделями и выдачами — в админке: раздел «Плагины» → плагин V0 Farm → секция «Управление V0 Farm». Ключи тестовые и показываются полностью.',
    docs: 'Админ добавляет ключи вида «Bearer vcp_...» в группы, настраивает модели v0 и выдаёт группы конкретному пользователю, тарифу, всем админам или всем. При исчерпании баланса (402/429/409/5xx) ключ уходит в кулдаун на 31 день и возвращается в пул готовых автоматически; промпт продолжается на следующем готовом ключе, сессия IDE не прерывается. Ключ с 401/403 помечается disabled. Модели v0: v0-mini / v0-pro / v0-max / v0-max-fast, а также gateway-формат creator/model (anthropic/claude-opus-5, openai/gpt-5.6-sol, moonshotai/kimi-k3 и т.д.).',
    changelog: [
      { version: '1.2.0', date: '2025-08-10', notes: 'Перенесён из таба админки в плагин. Управление ключами — в редакторе плагина. Ключи показываются полностью (тестовые). Иконка Sprout.' },
    ],
